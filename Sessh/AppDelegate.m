...
- (void) awakeFromNib {
    ...
    sesshJSONPathPref = [NSHomeDirectory() stringByAppendingPathComponent:@".sessh.path"];
    
    //if file sessh.path exists in ~/.sessh.path then read this file as it should contain the custom path to *.json
    if( [[NSFileManager defaultManager] fileExistsAtPath:sesshJSONPathPref] ) {
        
        //Read the sessh.path file which contains the path to the json file
        NSString *jsonConfigPath = [NSString stringWithContentsOfFile:sesshJSONPathPref encoding:NSUTF8StringEncoding error:NULL];
        ...
        sesshConfigFile = jsonConfigPath;
    }else{
        // The path for the configuration file (by default: ~/.sessh.json)
        sesshConfigFile = [NSHomeDirectory() stringByAppendingPathComponent:@".sessh.json"];
        
        // if the config file does not exist, create a default one
        if ( ![[NSFileManager defaultManager] fileExistsAtPath:sesshConfigFile] ) {
            NSString *cgFileInResource = [[NSBundle mainBundle] pathForResource:@"sessh.default" ofType:@"json"];
            [[NSFileManager defaultManager] copyItemAtPath:cgFileInResource toPath:sesshConfigFile error:nil];
        }
    }
...
- (NSDictionary<NSString *, NSDictionary *> *)parseSSHConfig:(NSString *)filepath {
    ...
    for (NSString *line in [fh componentsSeparatedByString:@"\n"]) {
        ...
        if (isComment && key && [first hasPrefix:@"sessh."]) {
...
- (void) loadMenu {
    ...
    NSData *data = [NSData dataWithContentsOfFile:sesshConfigFile];
    ...
    if (showSshConfigHosts) {
        ...
        for (NSString* key in servers) {
            ...
            NSMutableArray* itemList = sesshHosts;
    ...
    [self buildMenu:sesshHosts addToMenu:menu];
}
...
- (void) openHost:(NSMenuItem *) sender {
    ...
    //If the JSON file is set to use iTerm
        ...
        if( [iTermVersionPref isEqualToString:@"stable"]) {
            ...
            //we are using the default action of sessh... The active window in a new tab
...
- (IBAction)showImportPanel:(id)sender {
    ...
    if(tvarNSInteger == NSOKButton){
        ...
        [[NSFileManager defaultManager] moveItemAtPath:sesshConfigFile toPath: [NSHomeDirectory() stringByAppendingPathComponent:@".sessh.json.backup"] error: nil];
        
        NSURL * selectedFileUrl = [openPanelObj URL];
        //Import the selected file
        //NSLog(@"copy filename from %@ to %@",selectedFileUrl.path,sesshConfigFile);
        [[NSFileManager defaultManager] copyItemAtPath:selectedFileUrl.path toPath:sesshConfigFile error:nil];
        //Delete the old configuration file
        [[NSFileManager defaultManager] removeItemAtPath:[NSHomeDirectory() stringByAppendingPathComponent:@".sessh.json.backup"]  error: nil];
    } else {
        return;
    }
...
- (IBAction)showExportPanel:(id)sender {
    ...
    if (result == NSFileHandlingPanelOKButton) {
        ...
        [[NSFileManager defaultManager] copyItemAtPath:sesshConfigFile toPath:saveURL.path error:nil];
    }
}
...
- (IBAction)configure:(id)sender {
    ...
    if([editorPref rangeOfString:@"default"].location != NSNotFound) {
        ...
        [[NSWorkspace sharedWorkspace] openFile:sesshConfigFile];
    }
    else{
        //build the editor command
        NSString *editorCommand = [NSString stringWithFormat:@"%@ %@", editorPref, sesshConfigFile];
        
        //build the reprensented object. It's expecting menuCmd, termTheme, termTitle, termWindow, menuName
        NSString *editorRepObj = [NSString stringWithFormat:@"%@¬_¬%@¬_¬%@¬_¬%@¬_¬%@", editorCommand, nil, @"Editing sessh JSON", nil, nil];
